...
const recent = readLines
  .map((line) => JSON.parse(line) as { created_at: string; archived: boolean })
  .filter((x) => !x.archived && +new Date(x.created_at) > Date.now() - 30 * 24 * 60 * 60 * 1000);
const queryMs = performance.now() - startRead;

console.log(`[benchmark] generated=${total} write_ms=${writeMs.toFixed(2)} query_ms=${queryMs.toFixed(2)} matched=${recent.length}`);
